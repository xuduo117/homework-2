...
#for ctt in range(0,1):
    hdulist = fits.open(save_path+str(ctt)+'_subtract_Q6_shift.fits')
    PA=PA_all[ctt]
...
#    plt.savefig('../image/4/'+str(ctt)+'_subtract_psf_rot.png')
    ...
    plt.xlim([611-image_size,611+image_size])
    plt.ylim([470-image_size,470+image_size])
#    plt.savefig('../image/4/'+str(ctt)+'_subtract_psf_rot_crop.png')
    """
...
#fits.writeto('subtract_psf_rot_sum.fits',np.sum(image_all_rot,axis=0),overwrite=True)
#fits.writeto('subtract_psf_rot_median.fits',np.median(image_all_rot,axis=0),overwrite=True)

fits.writeto('subtract_psf_rot_sum_12.fits',np.sum(image_all_rot,axis=0),overwrite=True)
fits.writeto('subtract_psf_rot_median_12.fits',np.median(image_all_rot,axis=0),overwrite=True)

image_size=200
#image_size=150
#file_all=['subtract_shift_sum','subtract_shift_median','subtract_rot_sum','subtract_rot_median']
#file_all=['subtract_psf_rot_sum','subtract_psf_rot_median']
file_all=['subtract_psf_rot_sum_12','subtract_psf_rot_median_12']
