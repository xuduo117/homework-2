...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct  9 00:31:11 2017
...
#    hdulist = fits.open(save_path+str(ctt)+'_subtract_Q7_42B.fits')
    hdulist = fits.open(save_path+str(ctt)+'_subtract_Q7_12.fits')
    PA=PA_all[ctt]
...
#fits.writeto('subtract_psf_Q7_rot_sum.fits',np.sum(image_all_rot,axis=0),overwrite=True)
#fits.writeto('subtract_psf_Q7_rot_median.fits',np.median(image_all_rot,axis=0),overwrite=True)

fits.writeto('subtract_psf_Q7_rot_sum_12.fits',np.sum(image_all_rot,axis=0),overwrite=True)
fits.writeto('subtract_psf_Q7_rot_median_12.fits',np.median(image_all_rot,axis=0),overwrite=True)

image_size=200
#image_size=150
#file_all=['subtract_shift_sum','subtract_shift_median','subtract_rot_sum','subtract_rot_median']
#file_all=['subtract_psf_Q7_rot_sum','subtract_psf_Q7_rot_median']
file_all=['subtract_psf_Q7_rot_sum_12','subtract_psf_Q7_rot_median_12']
for ctt in range(len(file_all)):
